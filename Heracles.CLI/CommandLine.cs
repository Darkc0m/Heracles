...
namespace Heracles.CLI
{
    public static class CommandLine
    {
        ...
        public static Command ImportTextCommand() {
            var importScedic = new Command("scedic", "Import text from a .po file to a SCEDIC.bin file")
            {
                new Option<string>("--srcPath", "Path of the .po file", ArgumentArity.ExactlyOne),
                new Option<string>("--dirPath", "The directory where the SCEDIC.bin file will be saved", ArgumentArity.ExactlyOne),
            };
            importScedic.Handler = CommandHandler.Create<string, string>(TextCommands.importScedic);

            var importUIMess = new Command("uimess", "Import text from a .po file to a UIMess.bin file (do not confuse with UIMess.arc)")
            {
                new Option<string>("--srcPath", "Path of the .po file", ArgumentArity.ExactlyOne),
                new Option<string>("--dirPath", "The directory where the UIMess.bin file will be saved", ArgumentArity.ExactlyOne),
            };
            importUIMess.Handler = CommandHandler.Create<string, string>(TextCommands.importUIMess);

            var importDataDic = new Command("dic", "Import text from a .po file to a DataDictionary.bin file")
            {
                new Option<string>("--srcPath", "Path of the .po file", ArgumentArity.ExactlyOne),
                new Option<string>("--dirPath", "The directory where the DataDictionary.eng file will be saved", ArgumentArity.ExactlyOne),
            };
            importDataDic.Handler = CommandHandler.Create<string, string>(TextCommands.importDataDictionary);

            var importItem = new Command("item", "Import text from a .po file to an itemdata.bin file")
            {
                new Option<string>("--srcPath", "Path of the .po file", ArgumentArity.ExactlyOne),
                new Option<string>("--dirPath", "The directory where the itemdata.bin file will be saved", ArgumentArity.ExactlyOne),
            };
            importItem.Handler = CommandHandler.Create<string, string>(TextCommands.importItemdata);

            var importMons = new Command("mons", "Import text from a .po file to a monsdata_e.bin file")
            {
                new Option<string>("--srcPath", "Path of the .po file", ArgumentArity.ExactlyOne),
                new Option<string>("--dirPath", "The directory where the monsdata_e.bin file will be saved", ArgumentArity.ExactlyOne),
            };
            importMons.Handler = CommandHandler.Create<string, string>(TextCommands.importMonsdata);

            var importTutorial = new Command("tut", "Import text from a .po file to a tutorial.bin file (extracted from a .arc file)")
            {
                new Option<string>("--srcPath", "Path of the .po file", ArgumentArity.ExactlyOne),
                new Option<string>("--dirPath", "The directory where the tutorial.bin file will be saved", ArgumentArity.ExactlyOne),
            };
            importTutorial.Handler = CommandHandler.Create<string, string>(TextCommands.importTutorial);

            var importArm = new Command("arm", "Import text from .po files to a arm9.bin file")
            {
                new Option<string>("--srcPath", "Path with the .po files", ArgumentArity.ExactlyOne),
                new Option<string>("--dirPath", "The directory where the arm9.bin file will be saved", ArgumentArity.ExactlyOne),
            };
